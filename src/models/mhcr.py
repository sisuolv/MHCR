...
class MHCR(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.build_interaction_matrices(dataset)

        # Initialize user and item embeddings
        self.initialize_embeddings()

        # Load or create adjacency matrices for image, text, and video features
        self.initialize_feature_adj_matrices(config)
        ...
        self.initialize_feature_embeddings()

    def build_interaction_matrices(self, dataset):
        """Build interaction and adjacency matrices."""
        # Prepare interaction matrix in COO format
        self.interaction_matrix = dataset.inter_matrix(form='coo').astype(np.float32)
        self.adj = self.scipy_matrix_to_sparse_tensor(
            self.interaction_matrix, torch.Size((self.n_users, self.n_items))
        )

        # Build normalized adjacency matrices for GCN layers
        ...
        # Build the normalized adjacency matrix for user-item interactions
    ...
    def initialize_feature_adj_matrices(self, config):
        """Load or build adjacency matrices for image, text, and video features."""
        dataset_path = os.path.abspath(config['data_path'] + config['dataset'])
        self.image_adj_file = os.path.join(dataset_path, f'image_adj_{self.knn_k}_{self.sparse}.pt')
        self.text_adj_file = os.path.join(dataset_path, f'text_adj_{self.knn_k}_{self.sparse}.pt')
        self.video_adj_file = os.path.join(dataset_path, f'video_adj_{self.knn_k}_{self.sparse}.pt')

    def initialize_feature_embeddings(self):
        """Initialize embeddings for image, text, and video features if available."""
    ...
    def get_adj_mat(self):
        """Construct the normalized adjacency matrix for user-item interactions."""
    ...
    def get_norm_adj_mat(self):
        """Compute the normalized Laplacian matrix for GCN layers."""
    ...
    def load_or_build_adj(self, file_path, embedding, knn_k, sparse):
        """Load or build adjacency matrix for features."""
    ...
    def initialize_embedding(self, feat, embedding_dim, hyper_num, adj_file):
        """Initialize embeddings and adjacency matrix for a feature (image, text, or video)."""
    ...
    def process_features(self, embedding_weight, transform_layer, hyper_matrix):
        """Process features and compute item and user hyper embeddings."""
        # Transform features to desired embedding dimension
        features = transform_layer(embedding_weight)
        # Compute item hyper embeddings
        item_hyper = torch.mm(embedding_weight, hyper_matrix)
        # Compute user hyper embeddings based on adjacency matrix
    ...
    def forward(self, adj, train=False):
        """
        Forward propagation for training and inference.

        Args:
            adj (torch.sparse.FloatTensor): Adjacency matrix.
            train (bool): Whether the model is in training mode.

        Returns:
            Tuple of embeddings depending on the mode.
        """
        # Process features for image, text, and video
        if self.i_feat is not None:
            image_feats, i_image_hyper, u_image_hyper = self.process_features(
                self.image_embedding.weight, self.image_trs, self.image_hyper
            )

        if self.t_feat is not None:
            text_feats, i_text_hyper, u_text_hyper = self.process_features(
                self.text_embedding.weight, self.text_trs, self.text_hyper
            )

        if self.v_feat is not None:
            video_feats, i_video_hyper, u_video_hyper = self.process_features(
                self.video_embedding.weight, self.video_trs, self.video_hyper
            )

        # Multiply item embeddings by gate outputs
        image_item_embeds = torch.mul(self.item_id_embedding.weight, self.gate_module.forward_gate(image_feats))
        text_item_embeds = torch.mul(self.item_id_embedding.weight, self.gate_module.forward_gate(text_feats))
        video_item_embeds = torch.mul(self.item_id_embedding.weight, self.gate_module.forward_gate(video_feats))
        ...
        # HGNN Layers
        u_image_hyper_embs, i_image_hyper_embs = self.hgnnLayer(
            self.drop(i_image_hyper), self.drop(u_image_hyper), self.aggregate_user_item()[self.n_users:]
        )
        u_text_hyper_embs, i_text_hyper_embs = self.hgnnLayer(
            self.drop(i_text_hyper), self.drop(u_text_hyper), self.aggregate_user_item()[self.n_users:]
        )
        u_video_hyper_embs, i_video_hyper_embs = self.hgnnLayer(
            self.drop(i_video_hyper), self.drop(u_video_hyper), self.aggregate_user_item()[self.n_users:]
        )

        # Concatenate hyper embeddings
        ai_hyper_embs = torch.cat([u_image_hyper_embs, i_image_hyper_embs], dim=0)
        at_hyper_embs = torch.cat([u_text_hyper_embs, i_text_hyper_embs], dim=0)
        av_hyper_embs = torch.cat([u_video_hyper_embs, i_video_hyper_embs], dim=0)

        # Compute attention for common embedding
        ...
        sep_image_embeds = torch.mul(self.gate_module.forward_gate(content_embeds), image_embeds - common_embeds)
        sep_text_embeds = torch.mul(self.gate_module.forward_gate(content_embeds), text_embeds - common_embeds)
        sep_video_embeds = torch.mul(self.gate_module.forward_gate(content_embeds), video_embeds - common_embeds)
        ...
        if train:
            # Return embeddings and intermediate representations for loss calculation
            return ( all_embeddings_users,
    ...
    def bpr_loss(self, users, pos_items, neg_items):
        ...
        regularizer = 1.0 / 2 * (users.norm(2).pow(2) + pos_items.norm(2).pow(2) + neg_items.norm(2).pow(2))
    ...
    def contrastive_loss(self, emb1, emb2, emb3=None, all_emb=None, temperature=0.2):
        ...
        if emb3 is not None and all_emb is not None:
            ...
            ttl_score = (
                torch.exp(torch.matmul(norm_emb1, norm_all_emb.T) / temperature).sum(dim=1)
                + torch.exp(torch.matmul(norm_emb2, norm_all_emb.T) / temperature).sum(dim=1)
                + torch.exp(torch.matmul(norm_emb3, norm_all_emb.T) / temperature).sum(dim=1)
            )
    ...
    def calculate_loss(self, interaction):
        ...
        # Graph-hypergraph contrastive loss
...
class Gate(nn.Module):
    ...
    def forward_common(self, x):
        """Compute common attention scores."""
...
class HyperGNN(nn.Module):
    ...
    def forward(self, item_hyper, user_hyper, embeddings):
        """Forward propagation for HyperGNN."""
...
class GraphRankContrast(nn.Module):
    ...
    def forward(self, g1_emb, g2_emb):
        ...
        # Sort embeddings for consistency
        ...
        # Negative scores with shuffled embeddings
