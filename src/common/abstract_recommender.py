...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        self.i_feat, self.t_feat, self.v_feat = None, None, None
        if not config['end2end'] and config['is_multimodal_model']:
            dataset_path = os.path.abspath(config['data_path'] + config['dataset'])
            # if file exist?
            i_feat_file_path = os.path.join(dataset_path, config['image_feature_file'])
            t_feat_file_path = os.path.join(dataset_path, config['text_feature_file'])
            v_feat_file_path = os.path.join(dataset_path, config['video_feature_file'])
            if os.path.isfile(i_feat_file_path):
                self.i_feat = torch.from_numpy(np.load(i_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
            ...
            assert self.i_feat is not None or self.t_feat is not None or self.v_feat is not None, 'Features all NONE'
